/**
 * Helix.js Model Plugin v5.0
 * Formal AST + Sorted/Compound Indexes + Top-K Streaming
 */
  ...
  install(app, options = {}) {
    ...
    function createAndNode(children) {
      ...
      const deduped = flat.filter(n => {
        ...
        const key = `${n.field}|${n.op}|${JSON.stringify(n.value)}`;
    ...
    function compileCompareNode(node) {
      ...
      switch (op) {
        ...
        case 'instanceof': return (item) => { const v = accessFn(item); return v instanceof value; };
    ...
    function createHeap(capacity, comparator) {
      ...
      function siftUp(i) {
        while (i > 0) {
          const p = (i - 1) >>> 1;
          if (comparator(heap[i], heap[p]) >= 0) break;
    ...
    function tryIndex(node, indexes, indexesDirty) {
      ...
      if ((node.type === 'RANGE' || (node.type === 'COMPARE' && ['>', '>=', '<', '<='].includes(node.op)))) {
        ...
        const op = node.type === 'RANGE' ? node.op : node.op;
    ...
    function model(source = []) {
      ...
      const _markDirty = () => {
        ...
        state._subscribers.forEach(cb => { try { cb(); } catch (e) { console.error('Subscriber error:', e); } });
        state._effects.forEach(effect => { try { effect(); } catch (e) { console.error('Effect error:', e); } });
      };

      const _markQueryDirty = () => {
        state._dirty = true;
        state._cacheVersion++;
        state._asyncCacheVersion++;
      };
      ...
      function _applySliceSelect(result) {
        if (state.limit !== null) result = result.slice(state.offset, state.offset + state.limit);
      ...
      const api = {
        ...
        fresh() { return app.$model(data); },
        newQuery() { return app.$model(data); },
        clone() { return _cloneState(); },

        // --- AST Serialization ---
        toAst() { return astToJson(state._ast); },
        fromAst(json) {
          const m = app.$model(data);
          const ns = m[STATE];
        ...
        effect(callback) {
          ...
          state._effects.add(wrapped);
          const result = wrapped();
          return { result, unsubscribe: () => state._effects.delete(wrapped) };
        },
        subscribe(callback) {
        ...
        paginate(perPage = 15, page = 1) {
          const built = _buildSync();
          const total = built.length;
          const offset = (page - 1) * perPage;
          let result = built.slice(offset, offset + perPage);
          // Apply projection only — slice already done above, state.offset/limit must not double-slice
          const projection = buildProjection();
          if (projection.type !== 'none') {
            const projectFn = compileProjection(state);
            result = result.map(item => projectFn(item));
        ...
        min(field) {
          ...
          if (type === 'async') { return (async () => (await exec()).reduce((min, item) => { const v = Number(getVal(item)); return v < min ? v : min; }, Infinity)); }
          return exec().reduce((min, item) => { const v = Number(getVal(item)); return v < min ? v : min; }, Infinity);
        },
        max(field) {
          const { type, exec } = _dispatch();
          const getVal = accessor(field);
          ...
          if (type === 'async') { return (async () => (await exec()).reduce((max, item) => { const v = Number(getVal(item)); return v > max ? v : max; }, -Infinity)); }
          return exec().reduce((max, item) => { const v = Number(getVal(item)); return v > max ? v : max; }, -Infinity);
        },
        median(field) {
          const { type, exec, isAsync } = _dispatch();
          const getVal = field ? accessor(field) : (x) => x;
        ...
        doesntContain(valueOrCallback, key, value) { return !this.contains(valueOrCallback, key, value); },
        has(...keys) {
          const items = _buildSync();
        ...
        get(key, defaultValue) {
          ...
          if (isAsync || type === 'async-lazy') {
            return (async () => {
              if (key === undefined) {
                ...
                if (type === 'async') result = _applySliceSelect(result);
                return result;
              }
              const items = await _materialize(type, exec);
          ...
          if (key === undefined) {
            ...
            if (type !== 'lazy') result = _applySliceSelect(result);
            return result;
          }
          const items = type === 'lazy' ? [...exec()] : exec();
          if (Array.isArray(items) && !isNaN(Number(key))) return items[key] !== undefined ? items[key] : defaultValue;
        ...
        first() {
          ...
          if (type === 'async') {
            return (async () => {
              ...
              const sliced = _applySliceSelect(res);
              return sliced[0] || null;
            })();
          }
          if (type === 'async-lazy') {
            return (async () => {
          ...
          return _applySliceSelect(exec())[0] || null;
        },
        last() {
          const { type, exec, isAsync } = _dispatch();
          if (type === 'async') {
            return (async () => {
              const res = await exec();
              const sliced = _applySliceSelect(res);
              return sliced[sliced.length - 1] || null;
            })();
          }
          if (type === 'async-lazy') {
            return (async () => {
