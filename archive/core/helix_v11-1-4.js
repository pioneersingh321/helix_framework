...
function watchEffect(effectFn, options = {}) {
  ...
  function resolveExpression(val, ctx, { asBoolean = false, fallback = void 0, contextName = "expression" } = {}) {
    ...
    } else if (globalConfig.allowInlineExpressions) {
      try {
  ...
  function createBuiltinDirectives(appConfig) {
    ...
    dirs.bind = {
      ...
      updated(el, { value: val, arg, ctx, trackCleanup }) {
        ...
        // Safer object literal detection: must be valid JSON-like object syntax
        // JSON.parse validates structure, prevents false positives like "{foo}" or "}bar{"
        let isObjectLiteral = false;
        if (trimmed.startsWith("{") && trimmed.endsWith("}")) {
          try { JSON.parse(trimmed); isObjectLiteral = true; } catch { /* not a valid object literal */ }
        }
        if (isObjectLiteral && !appConfig.allowInlineExpressions) {
          return warn(`Inline expressions disabled. Cannot evaluate object literal: ${val}`);
        }
        const updateFn = () => {
          let result;
          if (isObjectLiteral) result = resolveExpression(val, ctx, { contextName: "v-bind object" });
          else result = resolvePath(val, ctx);
          if (arg === "class") {
  ...
  function createApp(rootComponent = {}) {
    ...
    const app = {
      ...
      use(plugin, options = {}) {
        ...
        if (typeof plugin.install === "function") plugin.install(pluginAPI, options);
        else if (typeof plugin === "function") plugin(pluginAPI, options);
        appPlugins.push({ plugin, options });
        return app;
      },
      provide(key, value) {
    ...
    // Run global plugins first, then app-specific plugins
    [...globalPlugins, ...appPlugins].forEach((p) => {
        if (p.plugin.install) p.plugin.install(pluginAPI, p.options);
        else if (typeof p.plugin === "function") p.plugin(pluginAPI, p.options);
    });
  ...
  function useGlobal(plugin, options = {}) {
    if (plugin && !globalPlugins.some((p) => p.plugin === plugin)) {
      if (typeof plugin.install === "function") {
        plugin.install(globalAPI, options);
      } else if (typeof plugin === "function") {
        plugin(globalAPI, options);
      }
