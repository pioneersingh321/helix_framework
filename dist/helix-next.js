(function(exports) {
  ...
  const warn = (msg, ...args) => {
    if (globalConfig.debug)
      console.warn(`[Helix]: ${msg}`, ...args);
  };
  const handleError = (err, context, instance = null) => {
    console.error(`[Helix Error] 💥 Caught in ${context}:`, err);
    if (instance && instance.name)
      warn(`Crash in component <${instance.name}>:`, err);
    else if (instance && instance.root)
      warn(`Crash in component:`, instance.root);
    if (globalConfig.rethrowErrors !== false)
      throw err;
  };
  ...
  function resolveExpression(val, ctx, { asBoolean = false, fallback = void 0, contextName = "expression", forceExpression = false } = {}) {
    ...
    if (exists) {
      ...
      if (globalConfig.debug && globalConfig.allowInlineExpressions && !warnedInlineExpressions.has(val)) {
        ...
        warn(`Security: inline expressions enabled. Never use with untrusted user input. (expression: "${val}")`);
      }
      try {
  ...
  function createBuiltinDirectives(appConfig) {
    ...
    dirs.for = {
      mounted(el, { value: val, ctx, instance, trackCleanup, bindNode: bindNode2 }) {
        ...
        if (!match)
          return warn(`[for] Invalid syntax: ${val}`);
          ...
          if (attr.name.startsWith("@") || attr.name.startsWith(`${appConfig.prefix}on:`)) {
            ...
            listeners[evtName].push((...args) => {
              ...
              if (typeof targetFn === "function")
                ...
                warn(`Inline expressions disabled. Cannot execute handler: ${attr.value}`);
            });
          } else {
        ...
        if (isDir) {
          ...
          if (dirDef) {
            ...
            try {
              ...
              console.error(`[Helix] 💥 Directive Error:`, err);
      ...
      directiveBindings.forEach(({ dirName, node: el, binding, normalized }) => {
        if (normalized.beforeUpdate || normalized.updated) {
          const updateEffect = effect(() => {
            if (binding.arg)
              resolvePath(binding.value, binding.ctx);
          }, {
            scheduler: () => {
  ...
  function computed$1(getterOrOptions) {
    ...
    if (typeof getterOrOptions === "function") {
      ...
      setter = () => warn(`[Helix] 💥 Write operation failed: computed value is readonly.`);
    } else {
      getter = getterOrOptions.get;
      setter = getterOrOptions.set || (() => warn(`[Helix] 💥 Write operation failed: no setter provided.`));
    }
    let value;
    let dirty = true;
    let hasError = false;
