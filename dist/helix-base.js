/**
 * Helix.js v11.1.14 - Grandmaster Release (Zenith Edition)
 * (c) 2026
 *
 * v11.1.14 patch:
 *   - REVERTED: hx-scope is no longer a builtin core directive. Removed
 *     dirs.scope (added in v11.1.8) entirely from createBuiltinDirectives.
 *     hx-scope goes back to being the standalone helix-scope.js plugin,
 *     registered via Helix.directive('scope', {...}) at the app level.
 *     resolveDirective's appDirectives -> globalDirectives -> builtinDirectives
 *     resolution order means the standalone plugin works exactly as it did
 *     before v11.1.8 with zero further changes needed. Everything else from
 *     v11.1.9-v11.1.13 (hx-text/hx-html/hx-bind fallback expressions, the
 *     resolveExpression null-mid-path and primitive-'in' fixes) is
 *     untouched - those are directive/shared-infra fixes independent of
 *     hx-scope's location.
 *
 * v11.1.13 patch (CRITICAL FIX):
 *   - resolveExpression: the existence-check loop's `parts[i] in current`
 *     test now guards against `current` being a non-null primitive
 *     (string/number/boolean), not just null/undefined. The `in` operator
 *     throws a TypeError on ANY primitive right-hand side - e.g. for
 *     `hx-if="state.charText.length > 50"`, getPathParts naively splits
 *     this into ["state", "charText", "length > 50"]; once the walk
 *     reaches "charText" (a real string value), the next check
 *     `"length > 50" in current` threw immediately, uncaught, inside the
 *     loop itself (no try/catch there - only the eval branch has one).
 *     Same underlying fragility class as v11.1.10 (a naive path-walk
 *     assumption breaking on real-world data), different trigger
 *     (primitive intermediate value vs. null intermediate value). Fixed
 *     by treating any non-object/function `current` the same as
 *     null/undefined: exists becomes false without throwing, val is
 *     correctly recognized as a non-simple-path (contains operators/
 *     whitespace) by isSimplePathSyntax, and falls through to the normal
 *     eval branch exactly as intended.
 *
 * v11.1.12 patch:
 *   - hx-bind:<attr>: the plain (non-object-literal) branch now falls back
 *     to resolveExpression instead of plain resolvePath (Round 3 of the
 *     fallback rollout). The :class/:style object-literal branches were
 *     already on resolveExpression and are untouched. Plain paths behave
 *     identically; expressions like hx-bind:src="user.avatar ?? '/default.png'"
 *     and hx-bind:disabled="user?.isLoading ?? false" now work when
 *     allowInlineExpressions is true. Protected against the v11.1.10
 *     null-mid-path crash from day one.
 *   - Confirmed (not a new finding, verified explicitly this round):
 *     warn()'s "Path not found" message is gated behind globalConfig.debug
 *     (captured once at load as __DEV__), so this rollout introduces no
 *     new console noise in any non-debug deployment - resolvePath never
 *     warned either way, and resolveExpression's warn only fires when
 *     debug was true at load time.
 *
 * v11.1.11 patch:
 *   - hx-html: now falls back to resolveExpression instead of plain
 *     resolvePath (Round 2 of the fallback rollout, same change as
 *     v11.1.9's hx-text). Plain paths behave identically; expressions
 *     like hx-html="object.bio ?? '<em>No bio</em>'" now work when
 *     allowInlineExpressions is true. Protected against the v11.1.10
 *     null-mid-path crash from day one, since it uses the already-fixed
 *     resolveExpression.
 *   - SECURITY NOTE: hx-html already renders raw HTML via sanitizeHtml();
 *     enabling allowInlineExpressions now also lets an hx-html expression
 *     be evaluated as JS. Both were already independently guarded by the
 *     same allowInlineExpressions flag and sanitizeHtml() respectively -
 *     this round doesn't widen either guard, but the combination is worth
 *     being deliberate about if allowInlineExpressions is ever enabled in
 *     an app that also renders any user-influenced content via hx-html.
 *
 * v11.1.10 patch (CRITICAL FIX):
 *   - resolveExpression: a bare property path (e.g. "store.candidate.full_name")
 *     that goes null/undefined partway through is no longer re-evaluated as
 *     raw JS. Previously, once the manual path-walk found the value missing,
 *     allowInlineExpressions=true caused it to fall to `new Function(...)`
 *     with the literal path text - which has no optional-chaining safety and
 *     throws on the exact null it just found. Worse, handleError's default
 *     rethrowErrors:true let that throw escape the effect, and flushJobs'
 *     job loop has no per-job try/catch, so a later-queued job (e.g. a
 *     sibling v-if toggle) could be skipped entirely for that flush cycle.
 *     Fix: a genuine bare path (no operators/calls/whitespace) now always
 *     falls back quietly instead of being eval'd, since eval offers nothing
 *     over the safe manual traversal for a plain path. Non-path expressions
 *     (`??`, ternaries, `?.`, etc.) are unaffected and still evaluate
 *     exactly as before. Fixes a regression surfaced by v11.1.9's hx-text
 *     change, though the underlying risk pre-dates it and also applied to
 *     hx-if/hx-show/{{ }} interpolation.
 *
 * v11.1.9 patch:
 *   - hx-text: now falls back to resolveExpression (same mechanism as
 *     hx-if/hx-show/{{ }} interpolation) instead of plain resolvePath.
 *     Plain paths behave identically; expressions like
 *     hx-text="object.count ?? 0" now work when allowInlineExpressions is
 *     true. With allowInlineExpressions false (default), behavior is
 *     byte-identical to before: unresolved paths warn and render "".
 *
 * v11.1.8 patch:
 *   - NEW: hx-scope:<name>="expression" builtin directive. Async-scoped
 *     reactive state ($loading/$error/$data/refresh + spread result keys)
 *     available to all descendant hx-* directives, custom directives/
 *     plugins, and {{ }} text interpolation, via the normal ctx chain.
 *   - hx-scope: optional ${prefix}scope-default:<name>='{"key":val}' JSON
 *     attribute supplies fallback values for keys missing/undefined on the
 *     resolved result (e.g. object.status defaults to false).
 *   - hx-scope: initial run() is deferred to the root instance's
 *     hooks.mount (not fired mid-bind), so expressions can safely rely on
 *     onMount()-initialized state elsewhere in the app.
 *   - hx-scope: a compile-error in the expression now still binds children
 *     against the parent ctx (fallback), instead of leaving the whole
 *     subtree unbound.
 *
 * v11.1.7 patch:
...
window.Helix = (function () {
    ...
    // A plain dotted/bracket property path, e.g. "store.candidate.full_name"
    // or "items[0]['key']" - no operators, calls, spaces, or optional-chaining
    // punctuation. Used to tell "this needs real JS semantics" apart from
    // "this is just a bare path that happens to be null/undefined right now".
    ...
    function resolveExpression(val, ctx, { asBoolean = false, fallback = void 0, contextName = "expression", forceExpression = false } = {}) {
        ...
        for (let i = 0; i < parts.length; i++) {
            // The `in` operator throws a TypeError on ANY primitive right-hand
            // side (string/number/boolean), not just null/undefined - e.g. once
            // `current` becomes a string mid-path (state.charText.length), the
            // next `"length > 50" in current` check would throw rather than
            // just meaning "there's nothing further to walk into". Treat any
            // non-object/function `current` the same as null/undefined here.
        ...
        if (exists) {
            ...
            // Bare property path that's missing, or went null/undefined partway
            // through (e.g. store.candidate.full_name after candidate is reset to
            // null). Evaluating this exact literal as raw JS would just repeat the
            // same access without the optional-chaining safety this loop already
            // applied, and can throw on a transient null. There's no expression to
            // gain from eval here - but we still need to register this as a
            // reactive dependency (the `in` check above never touches the get
            // trap, so it never calls track()), so replay the same path with a
            // null-safe traversal purely to establish tracking on whichever keys
            // do exist along the way, discarding the result.
            try {
                parts.reduce((acc, part) => acc?.[part], ctx);
            } catch (e) { /* tracking only - never throw from here */ }
