(function(exports) {
  ...
  function trimDeps(effectFn) {
    if (effectFn.deps && effectFn.deps.size > MAX_DEPS_SIZE) {
      ...
      effectFn.deps.clear();
      depsArray.slice(0, MAX_DEPS_SIZE / 2).concat(depsArray.slice(-MAX_DEPS_SIZE / 2)).forEach((dep) => effectFn.deps.add(dep));
    }
  }
  ...
  const VERSION = typeof { url: _documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === "SCRIPT" && _documentCurrentScript.src || new URL("helix.js", document.baseURI).href } !== "undefined" && { "VITE_CORE_VERSION": "11.1.15", "VITE_AXIOS_DEFAULT_URL": "/", "VITE_AXIOS_VERSION": "2.2.1", "VITE_DIRECTIVES_VERSION": "2.2.0", "VITE_FETCH_VERSION": "2.8.7", "VITE_HELPERS_VERSION": "1.1.0", "VITE_LOADER_VERSION": "2.5.1", "VITE_MODEL_VERSION": "2.2.1", "VITE_NOTIFY_VERSION": "2.2.6", "VITE_SCOPE_VERSION": "2.0.2", "VITE_TOOLTIP_VERSION": "1.7.0", "BASE_URL": "/", "MODE": "production", "DEV": false, "PROD": true, "SSR": false } && "11.1.15" || "11.1.15";
  ...
  function isRaw(value) {
    ...
    return value instanceof Node || value instanceof Event || value instanceof NodeList || value instanceof HTMLCollection || value instanceof DOMTokenList || value instanceof Window || value instanceof Document || value instanceof CSSStyleDeclaration || value instanceof Promise;
  }
        ...
        const updateFn = () => {
          ...
          list.forEach((item, index) => {
            ...
            if (node) {
              ...
              if (indexName)
                node.__hx_scope[indexName] = index;
            } else {
    ...
    function bindNode(node, ctx, instance, cleanupTarget, force = false) {
      ...
      if (compDef) {
        ...
        try {
          ...
          const setupCtx = new Proxy(baseSetupCtx, {
      ...
      if (node.hasAttribute(`${appConfig.prefix}for`)) {
        ...
        if (dir) {
          const binding2 = { value: val, ctx, instance, trackCleanup, bindNode };
          const hook = createDirectiveHook("for", "mounted", node, binding2, instance, normalizeDirective(dir));
          if (hook)
            hook();
        }
        return;
      ...
      if (node.hasAttribute(`${appConfig.prefix}if`)) {
        ...
        if (dir) {
          const binding2 = { value: val, ctx, instance, trackCleanup, bindNode };
          const hook = createDirectiveHook("if", "mounted", node, binding2, instance, normalizeDirective(dir));
          if (hook)
            hook();
        }
        return;
      ...
      collectedDirectives.forEach(({ attr, dirName, arg, modifiers, dirDef }) => {
        ...
        try {
          ...
          const mountedHook = createDirectiveHook(dirName, "mounted", node, binding2, instance, normalizeDirective(dirDef));
          if (mountedHook) {
          ...
          if (normalized.updated || normalized.unmounted) {
            directiveBindings.push({ dirName, node, binding: binding2, normalized });
          }
      ...
      directiveBindings.forEach(({ dirName, node: el, binding: binding2, normalized }) => {
        if (normalized.beforeUpdate || normalized.updated) {
          const updateEffect = effect(() => {
            if (binding2.arg)
              resolvePath(binding2.value, binding2.ctx);
          }, {
            name: `directive update: ${dirName}`,
            area: "directive",
            scheduler: () => {
              binding2._oldValue = resolvePath(binding2.value, binding2.ctx);
              const beforeUpdateHook = createDirectiveHook(dirName, "beforeUpdate", el, binding2, instance, normalized);
              if (beforeUpdateHook)
                beforeUpdateHook();
              const updatedHook = createDirectiveHook(dirName, "updated", el, binding2, instance, normalized);
              if (updatedHook)
                updatedHook();
            },
            lazy: false
        ...
        if (normalized.beforeUnmount || normalized.unmounted) {
          trackCleanup(() => {
            const beforeUnmountHook = createDirectiveHook(dirName, "beforeUnmount", el, binding2, instance, normalized);
            if (beforeUnmountHook)
              beforeUnmountHook();
            const unmountedHook = createDirectiveHook(dirName, "unmounted", el, binding2, instance, normalized);
            if (unmountedHook)
              unmountedHook();
          });
        }
  ...
  function inspect(node) {
    ...
    return {
      ...
      bindingMetadata: node.__hx_binding || null
    };
  }
  ...
  function checkMemoryLeaks() {
    ...
    activeEffectRegistry.forEach((eff) => {
      if (!eff._registeredInstances || eff._registeredInstances.size === 0) {
        leaks.push({
          id: eff.id,
          name: eff.name || "Anonymous Effect",
