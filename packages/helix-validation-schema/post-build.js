...
const packageName = packageJson.name; // "helix-validation-schema"
const folderName = packageName.replace('helix-', ''); // "validation-schema"
...
function processFile(src, dest) {
    if (fs.existsSync(src)) {
        ...
        code = code.replace(/const root\s*=\s*(typeof window !== ['"]undefined['"]\s*\?\s*window\s*:\s*globalThis);\s*root\.HelixValidationSchemaPlugin\s*=\s*HelixValidationSchemaPlugin;/g, '');
        code = code.replace(/const root\s*=\s*\(typeof window !== ['"]undefined['"]\s*\?\s*window\s*:\s*globalThis\);\s*root\.HelixValidationSchemaPlugin\s*=\s*HelixValidationSchemaPlugin;/g, '');

        // Replace IIFE wrapper footers
        const targetWrapper = '})(this.HelixValidationSchemaPlugin = this.HelixValidationSchemaPlugin || {});';
        const targetWrapperMin = '})(this.HelixValidationSchemaPlugin=this.HelixValidationSchemaPlugin||{});';
    ...
    root.HelixValidationSchemaPlugin = Object.assign(HelixValidationSchemaPlugin, exports);
        ...
        const newWrapperMin = `var root=typeof window!=="undefined"?window:globalThis;root.HelixValidationSchemaPlugin=Object.assign(HelixValidationSchemaPlugin,exports)})(typeof window!=="undefined"?window.HelixValidationSchemaPlugin=window.HelixValidationSchemaPlugin||{}:globalThis.HelixValidationSchemaPlugin=globalThis.HelixValidationSchemaPlugin||{});`;
