...
export function adapter(name, fn) {
...
export function yupAdapter(yupSchema) {
    return async (values) => {
        try {
...
export function ajvAdapter(ajvValidateFn) {
    return async (values) => {
        ...
        (ajvValidateFn.errors || []).forEach(err => {
            const path = (err.instancePath ? err.instancePath.slice(1).replace(/\//g, '.')
                       : err.dataPath ? err.dataPath.slice(1).replace(/\//g, '.') : '')
...
// Auto-register default adapters into the registry
adapters.add('zod', zodAdapter);
adapters.add('yup', yupAdapter);
adapters.add('ajv', ajvAdapter);

// Plugin installation definition to register globally
const HelixValidationSchemaPlugin = {
    ...
    install(app) {
        ...
        if ($validation) {
            // Attach schema support to the validation object if installed
...
export default HelixValidationSchemaPlugin;
